...
> **Canonical doc.** Where this file and `www.tokens.xyz-DESIGN.md` disagree, this one wins.
> Section 0 documents the tokens as actually implemented in the prototype (`code/`); the
> remaining sections are the aspirational reference they were derived from.

## 0. As-Implemented Tokens (Prototype)

The prototype (`code/app/globals.css`) implements the system as CSS variables. These are the
source of truth for what is on screen:

The ink/border scales mirror the live site's compiled tokens: text derives from
`gray-1400` (`#0E0E0F`) at 100/72/58/44% opacity steps; borders derive from
`gray-1300` (`#1F1F21`) at 20/12/4%.

| Variable | Value | Role |
|---|---|---|
| `--color-ink` | `#0E0E0F` | Primary text (live `text-extra-high`) |
| `--color-ink-muted` | `rgba(14, 14, 15, 0.72)` | Secondary text, labels (live `text-medium`) |
| `--color-ink-subtle` | `rgba(14, 14, 15, 0.58)` | Tertiary text, captions (live `text-low`) |
| `--color-ink-faint` | `rgba(14, 14, 15, 0.44)` | Axis ticks, rank numbers, arrows (live `text-extra-low`) |
| `--color-line` | `rgba(31, 31, 33, 0.12)` | Hairline borders (live `border-light`) |
| `--color-line-strong` | `rgba(31, 31, 33, 0.2)` | Emphasized/hover borders (live `border-medium`) |
| `--color-line-faint` | `rgba(31, 31, 33, 0.04)` | In-table row dividers (live `border-extra-light`) |
| `--color-surface-page` | `#FFFFFF` | Page background |
| `--color-surface-soft` | `rgba(14, 14, 14, 0.06)` | Pills, filter tracks, soft chips |
| `--color-surface-raised` | `#FFFFFF` | Cards and modules |
| `--color-hover-soft` | `#F5F5F8` | Row hovers, logo backdrops (live `gray-50`) |
| `--color-hover-soft-50` | `rgba(245, 245, 248, 0.5)` | Market-row hover tint, header tints |
| `--color-dark` | `#171717` | Dark fills (avatars, dark buttons) |
| `--color-up` | `#51C148` | Positive change, healthy grades, up-trend chart |
| `--color-down` | `#CF4040` | Negative change, weak grades, down candles |
| `--color-trend-down` | `#005288` | **Exception:** down-trend line/area uses SpaceX blue, not red |

Other as-built details that differ from the reference sections below:

- **Body typography:** weight 450, letter-spacing `-0.01em` (live uses `-0.025em`).
- **Type scale in use:** 10–14px labels/badges/body, **22px / weight 500** section headings
  (`title-md`), 28px page H1, 34px price display, 36px health gauge score. Weights cap at 600
  semibold (no 700). Numeric values use `InterVariable` with `tabular-nums` (`.num` utility).
- **Border radii in use:** `9999px` pills, `10px` variant avatars, **24px** buy-module rows,
  `22px` variant cards and modules (Buy, Market health), **`28px`** variants container shell,
  `19/27/29px` floating feed shells (matched to tokens.xyz).
- **Chart:** trend line is `1.5px` with 2px-radius corner smoothing (not 2px as stated in §9.12);
  fill is a 14%→0% gradient of the trend color.
- **Hover conventions:** variant cards deepen shadow + border (`.hv-variant-card`), market rows
  tint `--color-hover-soft-50` (`.hv-market-row`), buy rows use `--color-hover-soft` with
  `active:scale(0.99)` (`.hv-buy-row`), all at 0.15–0.2s ease.
...
12. **Data visualization (charts, sparklines):** Use success green (`#51C148`) for positive trends. Negative trend lines use SpaceX blue (`#005288`) in this prototype (see §0); negative candles and price-change badges use `#CF4040`. Line width: 1.5px with 2px corner smoothing. Grid lines: dashed `--color-line`. Tooltip background: `#2D2D2D`, text: `#FFFFFF`.